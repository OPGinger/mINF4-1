...
It defines a BTreeVis class that extends the BTree implementation to generate a PDF visualization of the tree structure after inserting values from a sequence.
...
def main() -> None:
    ...
    """ create context and load data from sequence """
    ctx = AlgoContext()
    sequence = "seq0"
    ...
    print(f"Leaf Depths: {leaf_depths[0]}")
    #print(f"All Keys: {keys_list}")
    print(f"Keys in Sorted Order: {is_sorted}")
    #print(f"Search Result for 0: {tree.search(0)}")
