...
aufgabe4_avl_loeschen.py

This script builds an AVL tree, performs delete operations,
and demonstrates that balancing is maintained after deletion.
"""
...
class NamedAVLTree(AVLTree):
    """AVLTree variant with configurable graph file base name."""
...
def latest_pdf_for_prefix(prefix: str) -> Path:
    """Return newest graph PDF matching the given filename prefix."""
...
def in_order_values(tree: AVLTree) -> list[int]:
    """Collect in-order traversal values as sorted-order verification."""
    out: list[int] = []
...
def main() -> None:
    ...
    ctx = AlgoContext()
    tree = NamedAVLTree(ctx, "AVL_Before_Delete")

    insert_values = [30, 20, 40, 10, 25, 35, 50, 5, 15, 27]
    delete_values = [50, 40, 35]
    ...
    before_pdf = latest_pdf_for_prefix("AVL_Before_Delete")

    for value in delete_values:
        tree.delete(value)

    tree.set_graph_filename("AVL_After_Delete")
    after_height = tree.root.height() if tree.root else 0
    after_in_order = in_order_values(tree)
    tree.graph_traversal()
    after_pdf = latest_pdf_for_prefix("AVL_After_Delete")

    answer_text = (
        "Task 4 - Deletion in AVL Tree\n"
        f"Inserted values: {insert_values}\n"
        f"Deleted values: {delete_values}\n"
        ...
        "Interpretation: after deletions, the AVL tree remains balanced due to rotations.\n"
    )
