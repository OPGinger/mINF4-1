...
szenario_blattknoten.py

Scenario 1: Delete a leaf node in an AVL tree.
"""
...
def main() -> None:
    ...
    delete_value = 50  # leaf node in this tree

    tree = NamedAVLTree(AlgoContext(), "AVL_Leaf_Before_Delete")
    ...
    before_pdf = latest_pdf_for_prefix("AVL_Leaf_Before_Delete")

    tree.delete(delete_value)
    tree.set_graph_filename("AVL_Leaf_After_Delete")

    after_height = tree.root.height() if tree.root else 0
    after_in_order = in_order_values(tree)
    tree.graph_traversal()
    after_pdf = latest_pdf_for_prefix("AVL_Leaf_After_Delete")

    answer_text = (
        "Task 4 - Scenario 1 (Leaf Node Deletion)\n"
