...
szenario_ein_kind.py

Scenario 2: Delete a node with exactly one child in an AVL tree.
"""
...
def main() -> None:
    ...
    delete_value = 25  # node with exactly one child (right child 27)

    tree = NamedAVLTree(AlgoContext(), "AVL_OneChild_Before_Delete")
    ...
    before_pdf = latest_pdf_for_prefix("AVL_OneChild_Before_Delete")

    tree.delete(delete_value)
    tree.set_graph_filename("AVL_OneChild_After_Delete")

    after_height = tree.root.height() if tree.root else 0
    after_in_order = in_order_values(tree)
    tree.graph_traversal()
    after_pdf = latest_pdf_for_prefix("AVL_OneChild_After_Delete")

    answer_text = (
        "Task 4 - Scenario 2 (One-Child Node Deletion)\n"
